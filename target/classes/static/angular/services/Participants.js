...
function AppParticipant(stream) {
	...
	that.videoID = "native-video-" + stream.getGlobalID();
        ...
        //buttonVideo.className = 'action btn btn--m btn--orange btn--fab mdi md-desktop-mac j-item-fullscreen';
	buttonVideo.className = 'btn btn--m btn--green btn--fab mdi md-fullscreen j-item-fullscreen';
	...
	buttonVideo.setAttribute("ng-click", "goFullscreen();$event.stopPropagation();");
        buttonVideo.setAttribute("lx-ripple", "");
        buttonVideo.style.position = "absolute";
        //buttonVideo.style.left = "75%";
        //buttonVideo.style.top = "60%";
	buttonVideo.style.right = "10px";
	buttonVideo.style.bottom = "10px";
        buttonVideo.style.zIndex = "100";
        that.videoElement.appendChild(buttonVideo);

	// hide/show camera
	var buttonCamera = document.createElement('button');
	that.actionClass = 'j-item-videocam-' + that.thumbnailId; 
	that.actionID = 'action-' + that.thumbnailId;
        buttonCamera.className = 'btn btn--m btn--green btn--fab mdi md-videocam-off ' + that.actionClass;
        //FIXME this won't work, Angular can't get to bind the directive ng-click nor lx-ripple
	buttonCamera.setAttribute("id", that.actionID);
        buttonCamera.setAttribute("ng-click", "vidoeCamShow();$event.stopPropagation();");
        buttonCamera.setAttribute("lx-ripple", "");
        buttonCamera.style.position = "absolute";
        buttonCamera.style.right = "50px";
        buttonCamera.style.bottom = "10px";
        buttonCamera.style.zIndex = "100";
        that.videoElement.appendChild(buttonCamera);
	$(document).off('click', '.' + that.actionClass);
        $(document).on('click', '.' + that.actionClass, function(e) {
	    var element = document.getElementById(that.actionID);
	    var elementVideo = document.getElementsByClassName(that.videoID);
	    if(!that.isVidoeCam){
		for(var i=0; i<elementVideo.length; i ++)
		    elementVideo[i].classList.remove("hidden");
		element.classList.remove("md-videocam");
                element.classList.add("md-videocam-off");
		that.isVidoeCam = true;
	    } else {
		for(var i=0; i<elementVideo.length; i ++)
		    elementVideo[i].classList.add("hidden");
		element.classList.remove("md-videocam-off");
                element.classList.add("md-videocam");
		that.isVidoeCam = false;
	    }
        });
	...
	if(stream.videoEnabled == true) 
	    $('#' + that.thumbnailID).removeClass('hidden');
	else 
            $('#' + that.thumbnailID).addClass('hidden');
    ...
    // checking streaming
    function checkingStream() {
        var elementVideo = document.getElementById(that.thumbnailId);
	console.log('vidoe streaming status -------------------->' + stream.getWrStream().getVideoTracks());
	if(stream.getWebRtcPeer().videoEnabled === true) {
	    elementVideo.classList.remove('hidden');
	} else {
	    elementVideo.classList.add('hidden');
	}
    }

    //var timerHandler = setInterval(checkingStream, 1000);
...
/*                "width": 22 + "%"
	...
	console.log("globalID ======>" + stream.getGlobalID());
